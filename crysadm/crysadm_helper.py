...
<DIV style="BACKGROUND-COLOR: #e6eae9">
    ...
    """
        ...
        r_session.set(extra_info_key,json.dumps(extra_info))
        r_session.setex('user_income_mail:%s' % user_info.get('username'),json_mail,3600*24)
...
def reboot_device(username, userid, dev):
    ...
    print('reboot')
    ubus_cd(account['session_id'], dev['account_id'], 'reboot', ["mnt", "reboot", {}], '&device_id=%s' % dev['device_id'])
                ...
                if not skip_clean and 'auto_clean_cache_enabled' in user_info.keys() and user_info['auto_clean_cache_enabled']:
                    if 'clean_trigger_limit' in user_info.keys() and 'clean_target_limit' in user_info.keys():
                        if int(dev['disk_quota'])!=10995116277760:
                            ...
                            if b_account is None: return
                            account = json.loads(b_account.decode('utf-8'))
                            ubus_cd(account['session_id'], dev['account_id'], 'set_quota', ["dcdn","set_quota",{"disk_space":"10995116277760"}], '&device_id=%s' % dev['device_id'])
                            ...
                            if (int(client['space_used'])==int(client['space_quota'])):
                                ...
                                ubus_cd(account['session_id'], dev['account_id'], 'set_quota', ["dcdn","set_quota",{"disk_space":"%d" % target}], '&device_id=%s' % dev['device_id'])
...
if __name__ == '__main__':
    ...
    while True:
        now = datetime.now()
        r_session.setex(('server_alive:%s' % collector_id), '在线', 120)
